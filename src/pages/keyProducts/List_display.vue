  ...
  <section class="list-display">
    ...
    <div class="operate-content clearfix">
      ...
      <div class="fr">
        ...
        <div class="operate-car-btn">
          <el-button type="primary" @click="addShopCart" size="mini" round>加入购物车</el-button>
        </div>
      </div>
    </div>
    <!-- 列表展示 -->
        ...
        >
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span v-if="scope.row.IsKg==='1'">
                可供
              </span>
              <span style="color:#ff2626" v-else>
                不可供
              </span>
            </template>
          </el-table-column>
        ...
        <el-checkbox-group v-model.trim="checkList" @change="handleCheckedItemChange">
          <div class="clearfix picture-content-layout">
            <div class="picture-content fl" v-for="(item, index) in tableData" :key="item.id">
              <el-checkbox ref="check" @change="singleChange(item.Id, item.Name, item, index)" :label="item.Id">
                <span></span>
              </el-checkbox>
              <el-popover trigger="hover" :open-delay="1000" placement="top">
                <HoverContent
              ...
              <h3>{{item.Name}}</h3>
              <h4>出版社名称：{{item.Id}}</h4>
              <h4>出版日期：{{item.pressDate}}</h4>
              <h4>是否可供：
                <span v-if="item.IsKg==='1'">可供</span>
                <span style="color: #ff2626" v-else>
                  不可供
                </span>
              </h4>
        ...
        for (let j = 0; j < this.tableData.length; j++) {
          if (selectAllIds[i] === this.tableData[j][idKey]) {
            if (!this.tabPictureSign) {
              ...
              this.tableData[j].JyCount = selectData[0].JyCount;
              this.tableData[j].XyCount = selectData[0].XyCount;
    ...
    changePageRecordData(oldVal) {
      ...
      this.multipleSelection.forEach(row => {
        ...
        if (selectAllIds.indexOf(row[idKey]) < 0) {
          ...
          selectData[0].JyCount = row.JyCount;
          selectData[0].XyCount = row.XyCount;
        }
      });
