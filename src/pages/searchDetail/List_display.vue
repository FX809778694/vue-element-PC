      ...
      <div v-else>
        ...
        >
          ...
          <el-table-column
            ...
            width="80"
          >
            <template slot-scope="scope">{{ scope.row.IsKg === "0" ? "否" : "是" }}</template>
          </el-table-column>
          <el-table-column
            prop="ChannelName"
            label="频道名称"
          ...
          <el-table-column
            ...
            width=""
          >
            <template slot-scope="scope">{{ scope.row.Publisher ? scope.row.Publisher : "暂无" }}</template>
          </el-table-column>
          <el-table-column
            prop="Author"
            label="作者"
            width=""
          >
            <template slot-scope="scope">{{ scope.row.Author ? scope.row.Author : "暂无" }}</template>
          </el-table-column>
          <el-table-column
            prop="FirstTime"
            label="首次到货时间"
            width="160"
          >
            <template slot-scope="scope">{{ scope.row.PunlishTime ? scope.row.PunlishTime : "暂无" }}</template>
          </el-table-column>
          <el-table-column
            prop="PublishTime"
            label="出版时间"
          ...
          <el-table-column
            ...
            width="116">
            <template slot-scope="scope">
              <div class="tab-num clearfix" v-if="!(scope.row.ChannelCode==5)">
                <el-input-number
              ...
              <div class="tab-num clearfix" v-else>
                <div class="rab-num-l">
                  <span>教用：</span>
                  <el-input-number
          ...
          <el-table-column fixed="right" label="详情" width="69">
            <template slot-scope="scope">
              <el-popover trigger="hover" :open-delay="1000" placement="top">
                ...
                <img
                  class="tab-btn"
                  @click="enterDetail(scope.row.Id, scope.row.ChannelCode)"
              ...
              <el-popover trigger="hover" :open-delay="1000" placement="top">
                ...
                >
                  ...
                  <div class="active-name"
                       v-if="!((item.ChannelCode==5)||(item.ChannelCode==4)||(item.ChannelCode==3)||(item.ChannelCode==2)||(item.ChannelCode==1))"
                  >
                    <p>活动：{{item.ChannelName}}</p>
                  </div>
                </div>
              </el-popover>
              <h3>{{item.Name}}</h3>
              <h4>出版社名称：{{item.Publisher}}</h4>
              <h4>出版时间：{{item.PublishTime}}</h4>
              <h4>是否可供：可供</h4>
              <h4>作者：{{item.Author}}</h4>
              <div class="picture-content-number" v-if="!(item.ChannelCode==5)">
                <el-input-number size="mini" :min="1" v-model.trim="item.Count"></el-input-number>
              </div>
              <div v-else>
                <div class="picture-content-number">
                  <span>教用：</span>
                  <el-input-number size="mini" :min="1" v-model.trim="item.JyCount"></el-input-number>
                </div>
                <div class="picture-content-number">
                  <span>学用：</span>
                  <el-input-number size="mini" :min="1" v-model.trim="item.XyCount"></el-input-number>
                </div>
              </div>
...
export default {
  ...
  computed: {
    ...mapState(["noBook", "clickSignAll", "clickSignSelect", "isStock"]),
    allSelect: function() {
  ...
  methods: {
    ...
    pictureActive() {
      ...
      this.changeAccount(this.tableData);
