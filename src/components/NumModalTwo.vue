    ...
    >
      <div class="batch-modal">
        <div class="batch-modal-item">
          <span>教用数量：</span>
          <el-input-number v-model.trim="batchNumJy" size="mini" @change="change" :min="1" label="请输入教用数量"></el-input-number>
        </div>
        <div class="batch-modal-item">
          <span>学用数量：</span>
          <el-input-number v-model.trim="batchNumXy" size="mini" @change="change" :min="1" label="请输入学用数量"></el-input-number>
        </div>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      confirmModal() {
        this.dialogVisible = false
        const { batchNumJy, batchNumXy } = this
        let value = {
